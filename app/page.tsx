      ...
      item: (
        <div className="">
          ...
          <p className="justify-evenly pb-6 leading-relaxed text-slate-700">
            I&apos;m <span className="font-bold">Jason,</span> a graduate
            software engineer based in{" "}
            <span className="font-bold">Melbourne </span>(Australia), currently
            seeking software engineer opportunities. You&apos;re on my website
    ...
    <div>
      ...
      {myItems.slice(0).map((item, index) => (
        <div
          key={index}
          style={{
            animation: `fadeIn 2s ease-in-out ${index * 1}s forwards`,
          }}
          className="opacity-0"
        >
          {item.item}
